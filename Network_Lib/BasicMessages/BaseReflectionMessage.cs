...
namespace Net
{
    ...
    public abstract class BaseReflectionMessage<T> : BaseMessage<T>
    {
        ...
        public override void SerializeHeader(ref List<byte> outData)
        {
            ...
            {
                byte[] serializedRoute = info.Serialize();
                outData.AddRange(serializedRoute);
            }
        }
        ...
        public override void DeserializeHeader(byte[] message)
        {
            ...
            int routeCount = BitConverter.ToInt32(message, offset);
            offset += sizeof(int);
            ...
            for (int i = 0; i < routeCount; i++)
            {
                ...
                RouteInfo info = RouteInfo.Deserialize(routeData);
                messageRoute.Add(info);
                offset += routeSize;
            }
